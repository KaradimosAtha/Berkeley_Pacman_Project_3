...
class CSP(search.Problem):
    ...
    dict of {var:val} entries:
        ...
        nconflicts(var, val, a) Return the number of other variables 
    ...
    def __init__(self, variables, domains, neighbors, constraints):
        ...
        #----------------------------------------------------------
        self.weight = {}#Με βαση το paper του dom/wdeg θα πρεπει να αρχικοποιησουμε τα weights καθε περιορισμου στο 1 
        #Επελεξα το weight να ειναι ενα dictionary με κλειδη ενα tuple της μορφης var_x-var_y οπου var_x και var_y ειναι δυο μεταβλητες που εμπλεκονται σε περιορισμο , και value θα ειναι αρχικα 1
        for var_x in self.variables:#Για καθε μεταβλητη var_x
            for var_y in self.variables:#Για καθε μεταβλητη var_y
                #Καθε μαθημα εχει περιορισμο με οποιοδηποτε αλλο , λογο του οτι εχουμε μονο μια αιθουσα και τρεια slots οπου σε καθε slot μπορει να μπει μονο ενα μαθημα
                if var_x!=var_y:
                    self.weight[(var_x , var_y)] = 1#Αρχικοποιουμε καθε weight σε 1 
        #-----------------------------------------------------------
...
def revise(csp, Xi, Xj, removals, checks=0):
    ...
    for x in csp.curr_domains[Xi][:]:
        ...
        if conflict:
            ...
            revised = True
    #---------------------------------------------        
    if not csp.curr_domains[Xi]: #Εαν το domain της Xi δεν εχει αλλες τιμες(δηλαδη ειναι αδειο) τοτε αυξανουμε το weight του περιορισμου Xi-Xj
        csp.weight[(Xi , Xj)]+=1
    #-----------------------------------------------
...
def dom_wdeg(assignment , csp):
    #Θελουμε να βρουμε την μικροτερη τιμη Dom/Wdeg ετσι ωστε να επιστρεψουμε την μεταβλητη που αντιστοιχει σε αυτη  , dom ειναι το domain της μεταβλητης(δηλαδη ποσες τιμες μπορει να παρει) και wdeg ειναι το αθροισμα ολων των weights των περιορισμων στους οποιους η μεταβλητη εμπλεκεται 
    min = math.inf #Αρχικοποιουμε μια μεταβλητη min στο + απειρο
    sum = {}#Οριζουμε ενα dictionary sum στο οποιο θα αποθηκευουμε για καθε μεταβλητη το αθροισμα των weights για καθε περιορισμο μεταξυ της μεταβλητης var και ενος γειτονα της neighbour
    return_var = None # Η μεταβλητη στην οποια θα αποθηκευσουμε το var που θα επιστρεψουμε 

    if csp.curr_domains==None: #Αν το curr_domains ειναι αδειο τοτε καλουμε την συναρτηση first_unassighed_variable η οποια επιλεγει μεταβλητη με βαση το default variable oreder
        return first_unassigned_variable(assignment, csp)
    
    for var in csp.variables:#Για καθε μεταβλητη του CSP
        if var not in assignment:#Εαν δεν της εχει ανατεθει ειδη τιμη 
            sum[var] =  0 #Αρχικοποιουμε το αθροισμα της στο 0 
            for neighbor in csp.neighbors[var]:#Περνουμε καθε γειτονα της μεταβλητης var
                if neighbor not in assignment:#Εαν δεν του εχει ανατεθει τιμη 
                    sum[var]+=csp.weight[(var , neighbor)]#Ενημεωνουμε το αθοισμα στης μεταβλητης var , προσθετοντας του την τιμη του weight για τον περιορισμο var-neighbor
            domain_len = len(csp.curr_domains[var])#Οταν βγουμε απο την for , περνουμε το πληθος απο τιμες που μπορει να παρει η var 
            if sum[var]!=0:#Ελεγχουμε εαν το αθροισμα ειναι διαφορο του μηδενος ετσι ωστε να μην κανουμε διαιρεση με το 0 
                if domain_len/sum[var] < min:#Με βαση το paper θα πρεπει να επιστρεψουμε την μεταβλητη με την μικροτερη τιμη dom/wdeg , αρα ελεγχουμε εαν αυτη ειναι μικροτερη απο το min
                    min = domain_len/sum[var] #Ενημερωνουμε καταλληλα το min
                    return_var = var#Και η μεταβλητη result γινεται ιση με το var 
            else:#Αλλιως θεωρουμε οτι το αθροισμα ειναι ισο με την μοναδα και υπολογιζουμε αντοιστιχα το min , ενημερωνοντας το return_var
                if domain_len < min:
                    min = domain_len
                    return_var = var
    if return_var !=None:#Τελος return_var δεν ειναι None , δηλαδη αλλαξε η τιμη του , τοτε το επιστρεφουμε
        return return_var
    else:#Αλλιως καλουμε την first_unassigned_variable
        return first_unassigned_variable(assignment , csp)
...
def forward_checking(csp, var, value, assignment, removals):
    ...
    for B in csp.neighbors[var]:
        if B not in assignment:
            ...
            if not csp.curr_domains[B]:#Σημαινει οτι το πεδιο τιμων του γειτονα B της μεταβλητης var ειναι αδειο αρα με βαση το paper θα πρεπει να αυξησουμε το βαρος για τον περιορισμο μεταξυ της B και του var   
                #------------------------------------
                csp.weight[(B , var)]+=1
                #------------------------------------
                return False
...
def mac(csp, var, value, assignment, removals, constraint_propagation=AC3):#Καλω τον AC3 και οχι τον AC3b γιατι αλλιως δεν θα καλεστει ποτε η revise(που ειναι και η συναρτηση στην οποια αυξανονται τα weights στο paper)
